...
async function main() {
    ...
    console.log('RESEARCH USING GOOGLE PLACE IDs - FIRST 10 TRIALS');
    console.log('='.repeat(70));
    console.log();

    const inputFile = '/Users/justin.holmes/Downloads/enriched_with_gplace_ids.csv';
    const outputFile = 'data/output/researched_with_place_ids_10.csv';
    ...
    // Process first 10 (excluding test accounts)
    const validTrials = trials
        .filter(t => t['Company / Account'].toLowerCase() !== 'your restaurant')
        .slice(0, 10);

    console.log(`🎯 Processing first 10 valid trials`);
    ...
    results.filter(r => r.actual_locations_found > 1).forEach(r => {
        console.log(`  ${r.company_name}: ${r.actual_locations_found} locations`);
    });
